...
public class OkxMarket implements Market {
    ...
    public Flux<Candle> fetchHistory(String symbol, int days) {
        ...
        System.out.println("[OKX MARKET] Fetching warm-up candle buffer from REST API for " + symbol);
        ...
        return webClient.get()
                ...
                .flatMapMany(response -> {
                    if (!"0".equals(response.get("code"))) return Flux.empty();
                    ...
                    return Flux.fromIterable(data)
                            .map(raw -> {
                                ...
                                return Candle.builder()
                                        .symbol(symbol).openTime(openTime).closeTime(openTime.plusMinutes(5))
                                        .open(new BigDecimal(raw.get(1))).high(new BigDecimal(raw.get(2)))
                                        .low(new BigDecimal(raw.get(3))).close(new BigDecimal(raw.get(4)))
                                        .volume(new BigDecimal(raw.get(5))).timeframe(Duration.ofMinutes(5))
                                        .build();
                            })
